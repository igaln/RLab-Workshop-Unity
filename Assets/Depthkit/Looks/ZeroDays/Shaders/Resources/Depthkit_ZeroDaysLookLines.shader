...
Shader "Depthkit/ZeroDaysLookLines"
{
    Properties
    {
    ...
    SubShader
    {
        // All Zero Days shaders are rendered in the transparency pass, with no shadowing
        ...
        Pass
        {
            ...
            void geom(line v2g input[2], inout TriangleStream<v2g> OutputStream)
            {
                ...
                if (depth0 <= (_NearClip + _ClipEpsilon) ||
                    depth0 >= (_FarClip  - _ClipEpsilon) ||
                    depth1 <= (_NearClip + _ClipEpsilon) ||
                    depth1 >= (_FarClip  - _ClipEpsilon))
                {
                    return;
                }
